...
try:
    ...
    class Image:
        @staticmethod
        def open(fp):
            raise Exception("PIL not installed")
...
def pack_assets(config: PackModelsConfig):
    ...
    print("DEBUG: Computing checksum...")
    combined_checksum = compute_checksum(combined_data)
    print("DEBUG: Checksum computed")
...
def process_assets_build(config_data):
    ...
    print("DEBUG: calling copy_assets")
    copy_assets(copy_config)
    print("DEBUG: calling pack_assets")
    pack_assets(pack_config)
    print("DEBUG: pack_assets finished")
...
if __name__ == '__main__':
    ...
    if args.merge:
        ...
        print("DEBUG: Starting build process")
