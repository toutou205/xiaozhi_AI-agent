...
void Application::Run() {
  const EventBits_t ALL_EVENTS =
      ...
      MAIN_EVENT_NETWORK_DISCONNECTED | MAIN_EVENT_TOGGLE_CHAT |
      MAIN_EVENT_START_LISTENING | MAIN_EVENT_STOP_LISTENING |
...
void Application::CheckNewVersion() {
  ...
  while (true) {
    ...
    if (ota_->HasActivationCode()) {
      ShowActivationCode(ota_->GetActivationCode(),
...
void Application::HandleToggleChatEvent() {
  auto state = GetDeviceState();
  ESP_LOGI(TAG, "HandleToggleChatEvent: Current state: %s",
  ...
  if (state == kDeviceStateIdle) {
    ESP_LOGI(TAG, "ToggleChat: Idle -> Connecting/Listening");
    ...
    ESP_LOGI(TAG, "ToggleChat: Speaking -> Abort");
    AbortSpeaking(kAbortReasonNone);
  } else if (state == kDeviceStateListening) {
    ESP_LOGI(TAG, "ToggleChat: Listening -> Idle");
    protocol_->CloseAudioChannel();
    SetDeviceState(kDeviceStateIdle);
  } else if (state == kDeviceStateConnecting) {
    ESP_LOGI(TAG, "ToggleChat: Connecting -> Idle");
    // Allow canceling the connection attempt
    if (protocol_) {
      protocol_->CloseAudioChannel();
    }
    SetDeviceState(kDeviceStateIdle);
...
void Application::HandleWakeWordDetectedEvent() {
  ...
  if (state == kDeviceStateIdle) {
    ...
    // Restart the activation check if the wake word is detected during
    // activation
    SetDeviceState(kDeviceStateIdle);
  }
}
...
void Application::HandleStateChangedEvent() {
  ...
  ESP_LOGI(TAG, "HandleStateChangedEvent: %s",
           state_machine_.GetStateName(new_state));
  clock_ticks_ = 0;
  ...
  case kDeviceStateListening:
    ...
    // Play popup sound after ResetDecoder (in EnableVoiceProcessing) has been
    // called
...
bool Application::UpgradeFirmware(const std::string &url,
  ...
  bool upgrade_success = Ota::Upgrade(upgrade_url, [display](int progress,
                                                             size_t speed) {
    std::thread([display, progress, speed]() {
      char buffer[32];
  ...
  if (!upgrade_success) {
    ...
    board.SetPowerSaveLevel(
        PowerSaveLevel::LOW_POWER); // Restore power save level
