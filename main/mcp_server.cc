...
void McpServer::AddUserOnlyTools() {
  ...
  // Debug tool for CPU Stats (Added by Xiaozhi AI)
  AddTool("self.debug.get_cpu_stats",
          "Analyze real-time CPU usage and provide optimization "
          "suggestions. This tool measures the CPU cycles consumed by "
          "each task to identify bottlenecks.",
          PropertyList(),
          [this](const PropertyList &properties) -> ReturnValue {
            // This returns a JSON string directly, which McpTool will
            // forward
            return SystemInfo::GetCpuStatsJson();
          });
