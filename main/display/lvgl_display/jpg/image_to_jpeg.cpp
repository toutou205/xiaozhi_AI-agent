...
static uint8_t *convert_input_to_encoder_buf(const uint8_t *src, uint16_t width,
                                             uint16_t height,
...
static uint8_t *convert_input_to_hw_encoder_buf(
    const uint8_t *src, uint16_t width, uint16_t height, v4l2_pix_fmt_t format,
...
static bool encode_with_hw_jpeg(const uint8_t *src, size_t src_len,
                                uint16_t width, uint16_t height,
                                v4l2_pix_fmt_t format, uint8_t quality,
  ...
  int enc_in_size = 0;
  uint8_t *enc_in = convert_input_to_hw_encoder_buf(
      src, width, height, format, &enc_src_type, &enc_in_size);
  if (!enc_in) {
  ...
  enc_cfg.sub_sample = (enc_src_type == JPEG_ENCODE_IN_FORMAT_GRAY)
  ...
  uint8_t *outbuf = (uint8_t *)jpeg_alloc_encoder_mem(
      out_cap, &jpeg_enc_output_mem_cfg, &out_cap_aligned);
  if (!outbuf) {
    free(enc_in);
  ...
  uint32_t out_len = 0;
  esp_err_t er = jpeg_encoder_process(s_hw_jpeg_handle, &enc_cfg, enc_in,
                                      (uint32_t)enc_in_size, outbuf,
                                      (uint32_t)out_cap_aligned, &out_len);
  free(enc_in);
...
static bool encode_with_esp_new_jpeg(const uint8_t *src, size_t src_len,
                                     uint16_t width, uint16_t height,
                                     v4l2_pix_fmt_t format, uint8_t quality,
                                     uint8_t **jpg_out, size_t *jpg_out_len,
                                     jpg_out_cb cb, void *cb_arg) {
  ...
  int enc_in_size = 0;
  uint8_t *enc_in = convert_input_to_encoder_buf(src, width, height, format,
  ...
  cfg.subsampling = (enc_src_type == JPEG_PIXEL_FORMAT_GRAY)
                        ? JPEG_SUBSAMPLE_GRAY
                        : JPEG_SUBSAMPLE_420;
  cfg.quality = quality;
  ...
  int out_len = 0;
  ret =
      jpeg_enc_process(h, enc_in, enc_in_size, outbuf, (int)out_cap, &out_len);
  jpeg_enc_close(h);
  jpeg_free_align(enc_in);
...
bool image_to_jpeg(uint8_t *src, size_t src_len, uint16_t width,
  ...
  if (format == V4L2_PIX_FMT_JPEG) {
    uint8_t *out_data = (uint8_t *)heap_caps_malloc(
        src_len, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    if (!out_data) {
...
#if CONFIG_XIAOZHI_ENABLE_HARDWARE_JPEG_ENCODER
  if (encode_with_hw_jpeg(src, src_len, width, height, format, quality, out,
                          out_len, NULL, NULL)) {
    return true;
  }
  // Fallback to esp_new_jpeg
#endif
  return encode_with_esp_new_jpeg(src, src_len, width, height, format, quality,
                                  out, out_len, NULL, NULL);
}
...
#endif // CONFIG_XIAOZHI_CAMERA_ALLOW_JPEG_INPUT
#if CONFIG_XIAOZHI_ENABLE_HARDWARE_JPEG_ENCODER
  if (encode_with_hw_jpeg(src, src_len, width, height, format, quality, NULL,
                          NULL, cb, arg)) {
    return true;
  }
  // Fallback to esp_new_jpeg
