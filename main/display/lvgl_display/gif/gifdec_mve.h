...
static inline void _gifdec_fill_bg_mve(uint8_t *dst, uint16_t w, uint16_t h,
                                       uint16_t stride, uint8_t *color,
                                       uint8_t opa) {
  lv_color32_t c =
      lv_color32_make(*(color + 0), *(color + 1), *(color + 2), opa);
  ...
  __asm volatile(".p2align 2                                             \n"
                 ...
                 "bne                 3b                                 \n"
                 : [dst] "+r"(dst), [h] "+r"(h)
                 : [src] "r"(color_32), [w] "r"(w),
                   [iTargetStride] "r"(stride * sizeof(uint32_t))
                 : "r0", "q0", "memory", "r14", "cc");
}

static inline void _gifdec_render_frame_mve(uint8_t *dst, uint16_t w,
                                            uint16_t h, uint16_t stride,
                                            uint8_t *frame, uint8_t *pattern,
                                            uint16_t tindex) {
  if (w == 0 || h == 0) {
    return;
  }
  ...
  __asm volatile(
      ...
      : [dst] "+r"(dst), [frame] "+r"(frame), [h] "+r"(h)
      : [pattern] "r"(pattern), [w] "r"(w), [stride] "r"(stride),
        [tindex] "r"(tindex)
