...
static gd_GIF *gif_open(gd_GIF *gif_base) {
  ...
  GIFDEC_FILL_BG(gif->canvas, gif->width * gif->height, 1,
                 gif->width * gif->height, bgcolor, 0x00);
#else
 ...
 * Return 0 on success or -1 on out-of-memory (w.r.t. LZW code table) or parse
 * error. */
...
static int read_image(gd_GIF *gif) {
  ...
  gif->fw = read_num(gif);
  gif->fh = read_num(gif);
  if (gif->fx + (uint32_t)gif->fw > gif->width ||
      gif->fy + (uint32_t)gif->fh > gif->height) {
...
#ifdef GIFDEC_FILL_BG
    GIFDEC_FILL_BG(&(gif->canvas[i * 4]), gif->fw, gif->fh, gif->width, bgcolor,
                   opa);
#else
    int j, k;
    for (j = 0; j < gif->fh; j++) {
