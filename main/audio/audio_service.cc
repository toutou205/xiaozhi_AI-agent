...
bool AudioService::ReadAudioData(std::vector<int16_t> &data, int sample_rate,
  ...
  if (!codec_->input_enabled()) {
    ESP_LOGI(TAG, "ReadAudioData: implicitly enabling input");
    esp_timer_stop(audio_power_timer_);
...
void AudioService::AudioInputTask() {
  while (true) {
    ...
    /* Used for audio testing in NetworkConfiguring mode by clicking the BOOT
     * button */
...
#endif
    if (debug_statistics_.playback_count % 100 == 0) {
      ESP_LOGI(TAG, "AudioOutputTask: Processed %d packets",
               debug_statistics_.playback_count);
    }
  }
...
void AudioService::SetDecodeSampleRate(int sample_rate, int frame_duration) {
  ...
  opus_decoder_.reset();
  opus_decoder_ =
      std::make_unique<OpusDecoderWrapper>(sample_rate, 1, frame_duration);
...
bool AudioService::PushPacketToDecodeQueue(
  ...
  if (audio_decode_queue_.size() >= MAX_DECODE_PACKETS_IN_QUEUE) {
    if (wait) {
      ...
      ESP_LOGW(TAG, "Decode queue full, dropping packet. Size: %d",
    ...
    while (seg_idx < page_segments) {
      ...
      if (!seen_head) {
        ...
        if (pkt_len >= 19 && std::memcmp(pkt_ptr, "OpusHead", 8) == 0) {
          ...
          // OpusHead结构：[0-7] "OpusHead", [8] version, [9] channel_count,
          // [10-11] pre_skip [12-15] input_sample_rate, [16-17] output_gain,
          // [18] mapping_family
          if (pkt_len >= 12) {
...
void AudioService::CheckAndUpdateAudioPowerState() {
  ...
  auto input_elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(
                           ...
                           .count();
  auto output_elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(
  ...
  if (output_elapsed > AUDIO_POWER_TIMEOUT_MS && codec_->output_enabled()) {
    // If input is enabled (e.g. Wake Word running), we must keep output enabled
    // to maintain the I2S clock/reference for AEC.
    if (!codec_->input_enabled()) {
