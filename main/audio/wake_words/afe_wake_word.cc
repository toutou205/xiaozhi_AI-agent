...
AfeWakeWord::AfeWakeWord()
    : afe_data_(nullptr), wake_word_pcm_(), wake_word_opus_() {
...
void AfeWakeWord::Feed(const std::vector<int16_t> &data) {
  if (afe_data_ == nullptr) {
    ESP_LOGE(TAG, "Feed: afe_data_ is null");
    return;
  }
  // ESP_LOGI(TAG, "Feed: %d samples", data.size()); // Too spammy, maybe once
...
void AfeWakeWord::AudioDetectionTask() {
  ...
  while (true) {
    ...
    if (res == nullptr || res->ret_value == ESP_FAIL) {
      if (loop_count++ % 100 == 0)
        ESP_LOGW(TAG, "AFE Fetch failed or null");
      continue;
      ;
    }
    if (loop_count++ % 300 == 0)
      ESP_LOGI(TAG, "AFE Running... Wakeup State: %d", res->wakeup_state);
...
void AfeWakeWord::StoreWakeWordData(const int16_t *data, size_t samples) {
  ...
  // keep about 2 seconds of data, detect duration is 30ms (sample_rate ==
  // 16000, chunksize == 512)
  ...
  wake_word_encode_task_ = xTaskCreateStatic(
      [](void *arg) {
        auto this_ = (AfeWakeWord *)arg;
        {
          ...
          auto encoder = std::make_unique<OpusEncoderWrapper>(
          ...
          for (auto &pcm : this_->wake_word_pcm_) {
            encoder->Encode(
                std::move(pcm), [this_](std::vector<uint8_t> &&opus) {
