...
// 添加类型别名
using ReturnValue =
    std::variant<bool, int, std::string, cJSON *, ImageContent *>;
...
public:
  ...
  Property(const std::string &name, PropertyType type, int min_value,
           int max_value)
      : name_(name), type_(type), has_default_value_(false),
        min_value_(min_value), max_value_(max_value) {
    if (type != kPropertyTypeInteger) {
      throw std::invalid_argument(
          "Range limits only apply to integer properties");
    }
  }
  ...
  std::string to_json() const {
    ...
    if (!required.empty()) {
      ...
      for (const auto &property : required) {
        cJSON_AddItemToArray(required_array,
                             cJSON_CreateString(property.c_str()));
      }
  ...
  std::string Call(const PropertyList &properties) {
    ...
    if (std::holds_alternative<ImageContent *>(return_value)) {
      ...
      if (std::holds_alternative<std::string>(return_value)) {
        ...
        cJSON_AddStringToObject(
            text, "text", std::to_string(std::get<int>(return_value)).c_str());
...
public:
  ...
  void AddTool(const std::string &name, const std::string &description,
               ...
               std::function<ReturnValue(const PropertyList &)> callback);
  void
  AddUserOnlyTool(const std::string &name, const std::string &description,
                  const PropertyList &properties,
                  std::function<ReturnValue(const PropertyList &)> callback);
