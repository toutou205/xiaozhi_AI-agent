...
void WifiBoard::StartWifiConfigMode() {
  ...
  xTaskCreate(
      [](void *arg) {
        ...
        audio_wifi_config::ReceiveWifiCredentialsFromAudio(&app, &wifi, disp,
                                                           ch);
        vTaskDelete(NULL);
      },
...
void WifiBoard::EnterWifiConfigMode() {
  ...
  if (state != kDeviceStateStarting) {
    ESP_LOGE(TAG,
             "EnterWifiConfigMode called but device state is not starting or "
             "speaking, device state: %d",
             state);
    return;
  }
