...
private:
  ...
  std::deque<float>
      state_buffer_; // Circular buffer for storing S[-1] and S[-2]
...
private:
  ...
  std::unique_ptr<FrequencyDetector>
      space_detector_; // Space frequency detector
...
public:
   ...
   */
  AudioSignalProcessor(size_t sample_rate, size_t mark_frequency,
                       size_t space_frequency, size_t bit_rate,
                       size_t window_size);
...
private:
  ...
  std::deque<uint8_t>
      identifier_buffer_;           // Buffer for start/end identifier detection
  ...
  const std::vector<uint8_t>
      start_of_transmission_; // Start-of-transmission identifier
  const std::vector<uint8_t>
      end_of_transmission_;         // End-of-transmission identifier
...
public:
   ...
   */
  AudioDataBuffer(size_t max_byte_size,
                  const std::vector<uint8_t> &start_identifier,
                  const std::vector<uint8_t> &end_identifier,
   ...
   */
  bool ProcessProbabilityData(const std::vector<float> &probabilities,
...
private:
   ...
   */
  std::vector<uint8_t>
  ConvertBitsToBytes(const std::vector<uint8_t> &bits) const;
