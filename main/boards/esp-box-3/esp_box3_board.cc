...
private:
  ...
  void InitializeButtons() {
    ...
    mute_button_.OnPressDown([this]() {
      ...
      // Add logic here if needed, e.g., Update UI icon
    });
    mute_button_.OnPressUp([this]() {
      ESP_LOGI(TAG, "Mute button state changed: Up (Mute Inactive)");
      // Hardware mute release leaves the codec in an undefined state.
      // We MUST force a software disable to re-trigger the hardware
      // initialization sequence (EnableInput(true)).
      auto codec = GetAudioCodec();
      codec->EnableInput(false);

      // If in Idle, we can also toggle Wake Word to be sure (optional but
      // safe).
      auto &app = Application::GetInstance();
      if (app.GetDeviceState() == kDeviceStateIdle) {
        ESP_LOGI(TAG, "Restoring Wake Word...");
  ...
  void InitializeIli9341Display() {
    ...
    const ili9341_vendor_config_t vendor_config = {
        ...
        .init_cmds_size =
            sizeof(vendor_specific_init) / sizeof(ili9341_lcd_init_cmd_t),
    };
    ...
    display_ =
        new emote::EmoteDisplay(panel, panel_io, DISPLAY_WIDTH, DISPLAY_HEIGHT);
#else
...
public:
  EspBox3Board()
      : boot_button_(BOOT_BUTTON_GPIO), mute_button_(MUTE_BUTTON_GPIO) {
    InitializeI2c();
    InitializeSpi();
