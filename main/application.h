...
#define MAIN_EVENT_GESTURE_DETECTED (1 << 13)
...
public:
   ...
   * It handles all events including network, state changes, and user
   * interactions.
   */
  void Run();
  ...
  /**
   * Inject a prompt text to be sent to the server as user input
   */
  void InjectPrompt(const std::string &prompt);

  /**
   * Reset protocol resources (thread-safe)
   * Can be called from any task to release resources allocated after network
   * connected This includes closing audio channel, resetting protocol and ota
   * objects
   */
  void ResetProtocol();
